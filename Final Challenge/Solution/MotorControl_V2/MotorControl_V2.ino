...
float sampleTime = 0.05;          //Sample time in Seconds
...
//Create Publishers
rcl_publisher_t motorSpeed_pub, motorCtrl_pub;

//Create Subscribers
rcl_subscription_t setPoint_sub;

//Define messages to be used
std_msgs__msg__Float32 setPoint_msg, motSpeed_msg, motCtrl_msg, ctrlSetpoint_msg;
    ...
    switch (state) {
      ...
      case AGENT_CONNECTED:
        ...
        if (state == AGENT_CONNECTED) {
            ...
            float dt = (float)(micros() - prev_time) / 1e6;
            if (dt > sampleTime) {
                    
              encoder_R.ReadSensors();                              // Read Encoders
              motorSpeed = encoder_R.Get_Speed();
                                      
              u = PID_R.GetControl(setPoint, motorSpeed);           //Get Control
              motor_R.MotorWrite(u);                                //Motor Write

              motSpeed_msg.data = motorSpeed;                       //Fill Messages
              motCtrl_msg.data = u;
          ...
          rclc_executor_spin_some(&executor, RCL_MS_TO_NS(1000*sampleTime/2.0));
        }
        break;
      ...
      case AGENT_DISCONNECTED:
        ...
        while (abs(motorSpeed) > 0.05){
          encoder_R.ReadSensors();                              // Read Encoders
          motorSpeed = encoder_R.Get_Speed();
          u = PID_R.GetControl(setPoint, motorSpeed);
